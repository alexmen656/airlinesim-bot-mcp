export const infoTools = [
    {
        ...
        description: "Ruft Informationen zur Airline ab. Zeigt Übersicht mit Airline-Name, Code, Hub, Land, Rating, Flottengröße, Mitarbeiter, aktuellem Kontostand, Finanzrating, Image-Ratings und Tochterunternehmen.",
        inputSchema: {
            type: "object",
            properties: {},
        },
...
export class InfoHandlers {
    ...
    async getAirlineInfo() {
        ...
        const data = await page.evaluate(() => {
            ...
            // Credits
            const creditsLink = document.querySelector('.as-footer-line-element a.credits');
            if (creditsLink) {
                const creditsSpan = creditsLink.querySelector('span.good');
                result.credits = creditsSpan ? creditsSpan.textContent.trim() + ' Credits' : '';
            }

            // Game end date
            const closeDate = document.querySelector('.as-footer-line-element.close-date');
            if (closeDate) {
                result.gameEndInfo = closeDate.textContent.trim();
            }
