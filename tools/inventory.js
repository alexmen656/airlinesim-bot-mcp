...
export class InventoryHandlers {
    ...
    async getRouteInventory(origin, destination) {
        ...
        const data = await page.evaluate(() => {
            ...
            const allTables = document.querySelectorAll('.as-panel .as-table-well table');
            let loadTable = null;

            for (const table of allTables) {
                const firstHeader = table.querySelector('thead tr th');
                if (firstHeader && firstHeader.textContent.trim() === 'SC') {
                    loadTable = table;
                    break;
                }
            }
